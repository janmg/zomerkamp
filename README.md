...
Project Structure
zomerkamp/
├── config.py               ← Database credentials & event constants
├── models.py               ← SQLAlchemy ORM models + DB helpers
├── requirements.txt
│
├── app1_import.py          ← CLI: import tasks & participants from CSV
├── app2_schedule.py        ← CLI: generate fair schedule, export CSV
├── app3_admin.py           ← CLI: admin / override management
│
├── app4_web/
...
The time block (morning / afternoon / evening) is derived automatically from the midpoint of begin/end.
...
Tasks are processed in day/time order.
For each task, eligible candidates are people who:
Are available in that day + time-block.
Have no Unavailability record blocking them.
Candidates are sorted by preference match first, then lowest accumulated points (fairness).
The first candidate becomes the lead (earns points); the rest are helpers (earn points).
One additional eligible person is assigned as backup (earns no points).
Export CSVs
Three files are written with a timestamp in the filename:

schedule_<ts>.csv — one row per assignment (all tasks × all people).
...
# Promote backup → helper (awards points) and select a new backup
...
Participant ─────────────────── Assignment ─── Task
  id, name, email, phone         id               id, name, day
  day1_morning … day4_evening    task_id          begin_time, end_time
  preference                     participant_id   points, people_required
  excluded_all_days              role             time_block
                                 points_awarded
